<template>
  ...
  <div class="form-group row">
    <label for="inputMatchID" class="col-sm-2 col-form-label">Match ID</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="inputMatchID" placeholder="00000"
      v-model="matchID">
    </div>
  </div>
  <div class="form-group row">
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control" id="inputTeam1" placeholder="team1" v-model="team1">
    </div>
  </div>
  <div class="form-group row">
    <label for="inputTeam2" class="col-sm-2 col-form-label">Team 2</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="inputTeam2" placeholder="team2" v-model="team2">
    </div>
  </div>
  <div class="form-group row">
    <label for="inputMom" class="col-sm-2 col-form-label">MoM</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="inputMom" placeholder="MoM" v-model="mom">
    </div>
  </div>
...
<script>
import ownersCollection from '../firebase-config'
...
export default { 
  ...
  methods:{
    ...
    async introduceMatchScore(){
      ...
      console.log(playersMatch.size+this.matchID+this.mom)
      console.log(this.getMatchScore(this.matchID))
      const matchScore = {
    "scorecard": [{
      "inningsId": 1,
      "batsman": [{
    ...
    "responseLastUpdated": "1648837358"
      ...
      this.addMatchScoreToDB(matchScore,this.matchID)
      // const addedDoc = await addDoc(ownersCollection, this.$data)
      // console.log(addedDoc)
      // alert("Scores added succesfully")
      // this.$router.push("/")
    },
    getMatchScore(id){
    ...
    addMatchScoreToDB(matchScore,matchId){
    ...
    Object.entries(matchScore.scorecard).forEach(item => {      
      if(item.batsman !== null){
        Object.entries(item[1].batsman).forEach(batsman => {
          ...
          this.playersScore.set(batsman[1].name,[this.getValidScore(batsman[1].runs),this.getValidScore(batsman[1].sixes),this.getValidScore(batsman[1].fours)])
    ...
    this.adjustNamefromNickName(this.playersBowling)
    this.adjustNamefromNickName(this.playersCatchingStumping)
    console.log(this.playersBowling)
    console.log(this.playersCatchingStumping)
    console.log(this.playersRunOuts)
  ...
  addOutDescrition(outDesc){
    ...
    if(outDesc !== undefined && outDesc !== 'not out'){  
      ...
      if(od.indexOf('st ') == 0){
        const keeper =od.substring(2,od.indexOf(' b'))
  ...
  adjustNamefromNickName(playerMap){
    playerMap.forEach((values,keys)=>{
        const playerNm = (this.playersNickName.get(keys)!== undefined ? this.playersNickName.get(keys) : keys)
        if(playerNm != keys){
          this.playersBowling.set(playerNm,values)
          this.playersBowling.delete(keys)
        }        
    })
